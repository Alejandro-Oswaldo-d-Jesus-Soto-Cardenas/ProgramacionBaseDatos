...
1 Primero lo que hacemos es descargar la imagen de  SQL Server 2022 para poder inicializar con nuestra implementación.
...
docker run -e 'ACCEPT_EULA=Y' -e 'MSSQL_SA_PASSWORD=Araceli123!' -p 1433:1433 --name sql_server_2022 -d mcr.microsoft.com/mssql/
server:2022-latest




3.Para verificar que mi contenedor de mi docker está corriendo y todo esta bien usamos este comando de verificación.
 docker ps



4. Después de verificar la conexión   vamos a ir    a verificar y conectar con nuestra base de datos  y completamos los datos que nos pide y damos conectar .
...
6. Aquí iniciamos y creamos un archivo y colocamos el código para crear  nuestra  primera Base de Datos .



7. De aquí creamos nuestras tablas como nos piden en la actividad.
CREATE TABLE seller (
...
8. De aquí realizamos las relaciones entre las tablas.
ALTER TABLE sale
...
Código completo del trabajo realizado:
SELECT @@VERSION;
